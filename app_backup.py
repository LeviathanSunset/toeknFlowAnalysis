...
class StreamlitTokenFlowApp:
    ...
    def render_all_addresses_table(self, analyzer):
        ...
        # 显示完整数据表
        st.dataframe(
            display_df[['排名', '地址/名称', '净流动(代币)', '净流动(美元)', '流入(代币)', '流出(代币)', '交易数', '类型']],
            width='stretch',
            height=800,
        ...
        with col1:
            net_inflow_count = len(df[df['net_tokens'] > 0])
            st.metric("📈 净流入地址", f"{net_inflow_count} 个")
        
        with col2:
            net_outflow_count = len(df[df['net_tokens'] < 0])
            st.metric("📉 净流出地址", f"{net_outflow_count} 个")
        
        with col3:
            total_net_tokens = df['net_tokens'].sum()
            st.metric("🪙 总净流动", self.format_tokens(total_net_tokens))
        
        with col4:
            total_net_value = df['net_value'].sum()
            st.metric("💰 总净价值", self.format_currency(total_net_value))
